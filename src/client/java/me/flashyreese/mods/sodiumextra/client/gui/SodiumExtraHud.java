...
public class SodiumExtraHud {
    ...
    public void onStartTick(Minecraft client) {
        ...
        if (SodiumExtraClientMod.options().extraSettings.showFps) {
            ...
            Component text = Component.translatable("vulkan-extra.overlay.fps", currentFPS);

            if (SodiumExtraClientMod.options().extraSettings.showFPSExtended)
                text = Component.literal(String.format("%s %s", text.getString(), Component.translatable("vulkan-extra.overlay.fps_extended", this.stats.getAverageFps(), this.stats.getOnePercentLowFps(),
        ...
        if (SodiumExtraClientMod.options().extraSettings.showCoords && this.client.player != null) {
            ...
            Component text = Component.translatable("vulkan-extra.overlay.coordinates", String.format("%.2f", pos.x), String.format("%.2f", pos.y), String.format("%.2f", pos.z));
            if (this.client.showOnlyReducedInfo()) {
                text = Component.translatable("vulkan-extra.overlay.coordinates_unavailable");
            }
        ...
        if (!SodiumExtraClientMod.options().renderSettings.lightUpdates) {
            Component text = Component.translatable("vulkan-extra.overlay.light_updates");
            this.textList.add(text);
        }
    }
